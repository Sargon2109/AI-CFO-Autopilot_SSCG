...
export default function RiskCard() {
  const { cashBalance, monthlyBurn } = useAppStore();

  // ✅ Use bank cash as the "reserve" so the progress bar moves when Bank amount changes
  const reserveForProgress = cashBalance;
  ...
  const projectedDepletion = useMemo(
    () => calcDepletionDateISO(runwayDays),
    [runwayDays]
  );

  const emergencyTarget = useMemo(
    () => monthlyBurn * targetMonths,
    [monthlyBurn]
  );

  const gap = useMemo(
    () => Math.max(0, emergencyTarget - reserveForProgress),
    [emergencyTarget, reserveForProgress]
  );

  const progressPct = useMemo(() => {
    if (emergencyTarget <= 0) return 0;
    return clamp((reserveForProgress / emergencyTarget) * 100, 0, 100);
  }, [reserveForProgress, emergencyTarget]);
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-3">
        ...
        <div className="rounded-xl bg-slate-50 p-4">
          ...
          <div className="mt-1 text-xs text-slate-600">
            Projected depletion: {projectedDepletion ?? "—"}
          </div>
        </div>
      </div>
        ...
        <div className="mt-4">
          ...
          <div className="mt-3 text-xs text-slate-600">
            ...
            <span className="font-semibold text-slate-900">
              {formatMoney(reserveForProgress)}
            </span>{" "}
            • Target:{" "}
            <span className="font-semibold text-slate-900">
              {formatMoney(emergencyTarget)}
            </span>
          </div>
        </div>
      </div>
